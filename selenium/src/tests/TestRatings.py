...
def getRatingsTest() :
    ...
    click_element_by_id("operations-tag-rating-controller")
    ...
    if status_code == '200':
        ...
        logger.error("Não foi possível realizar a função de Listar Avaliações: O sistema não possui dados!")
    
    else:
        raise Exception("Não foi possível realizar a função de Listar Avaliações")
