...
def getTicketsTest() :
    ...
    click_element_by_id("operations-tag-ticket-controller")
    ...
    if status_code == '200':
        logger.info("Tickets obtidos com sucesso!")
        logger.info("Corpo da resposta:\n " + response_body)
    
    elif response_body == "[]":
        logger.error("Não foi possível realizar a função de Listar Tickets: O sistema não possui dados!")
    
    else:
        raise ValueError("Não foi possível realizar a função de Listar Tickets")